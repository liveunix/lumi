...
def install_grub(partition_fs):
    ...
    if grub_i386_dir.exists():
        ...
        sh.contrib.sudo('grub-install', partition_fs[:-1], boot_directory=partition['mountpoint'] + '/boot', target='i386-pc', removable=True)
    else:
    ...
    if grub_efi_dir.exists():
        ...
        sh.contrib.sudo('grub-install', partition_fs[:-1], efi_directory=partition['mountpoint'], target='x86_64-efi', bootloader_id='lumi', removable=True)
    else:
